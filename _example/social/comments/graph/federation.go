...
func (ec *executionContext) resolveEntity(
	...
	case "Comment":
		...
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "Comment": %w`, err)
		}
		switch resolverName {
		...
		case "findCommentByID":
			...
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findCommentByID(): %w`, err)
			}
			entity, err := ec.resolvers.Entity().FindCommentByID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "Comment": %w`, err)
			}
	...
	case "Post":
		...
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "Post": %w`, err)
		}
		switch resolverName {
		...
		case "findPostByID":
			id0, err := ec.unmarshalNID2string(ctx, rep["id"])
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findPostByID(): %w`, err)
			}
...
func entityResolverNameForComment(ctx context.Context, rep EntityRepresentation) (string, error) {
	...
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for Comment", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for Comment", ErrTypeNotFound))
			break
		}
		return "findCommentByID", nil
	}
	return "", fmt.Errorf("%w for Comment due to %v", ErrTypeNotFound,
...
func entityResolverNameForPost(ctx context.Context, rep EntityRepresentation) (string, error) {
	...
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for Post", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for Post", ErrTypeNotFound))
			break
		}
