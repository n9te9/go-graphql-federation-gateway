...
func (ec *executionContext) resolveEntity(
	...
	switch typeName {
	case "BillingInfo":
		resolverName, err := entityResolverNameForBillingInfo(ctx, rep)
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "BillingInfo": %w`, err)
		}
		switch resolverName {
		...
		case "findBillingInfoByID":
			...
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findBillingInfoByID(): %w`, err)
			}
			entity, err := ec.resolvers.Entity().FindBillingInfoByID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "BillingInfo": %w`, err)
			}
	...
	case "Organization":
		...
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "Organization": %w`, err)
		}
		switch resolverName {
		...
		case "findOrganizationByID":
			id0, err := ec.unmarshalNID2string(ctx, rep["id"])
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findOrganizationByID(): %w`, err)
			}
...
func entityResolverNameForBillingInfo(ctx context.Context, rep EntityRepresentation) (string, error) {
	...
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for BillingInfo", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for BillingInfo", ErrTypeNotFound))
			break
		}
		return "findBillingInfoByID", nil
	}
	return "", fmt.Errorf("%w for BillingInfo due to %v", ErrTypeNotFound,
		errors.Join(entityResolverErrs...).Error())
}

func entityResolverNameForOrganization(ctx context.Context, rep EntityRepresentation) (string, error) {
	// we collect errors because a later entity resolver may work fine
	// when an entity has multiple keys
	entityResolverErrs := []error{}
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for Organization", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for Organization", ErrTypeNotFound))
			break
		}
