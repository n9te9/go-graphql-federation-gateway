...
func (ec *executionContext) resolveEntity(
	...
	switch typeName {
	case "Booking":
		resolverName, err := entityResolverNameForBooking(ctx, rep)
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "Booking": %w`, err)
		}
		switch resolverName {
		...
		case "findBookingByID":
			...
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findBookingByID(): %w`, err)
			}
			entity, err := ec.resolvers.Entity().FindBookingByID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "Booking": %w`, err)
			}
	...
	case "Flight":
		...
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "Flight": %w`, err)
		}
		switch resolverName {

		case "findFlightByNumberAndDepartureDate":
			id0, err := ec.unmarshalNString2string(ctx, rep["number"])
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findFlightByNumberAndDepartureDate(): %w`, err)
			}
			id1, err := ec.unmarshalNString2string(ctx, rep["departureDate"])
			if err != nil {
...
func entityResolverNameForBooking(ctx context.Context, rep EntityRepresentation) (string, error) {
	...
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for Booking", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for Booking", ErrTypeNotFound))
			break
		}
		return "findBookingByID", nil
	}
	return "", fmt.Errorf("%w for Booking due to %v", ErrTypeNotFound,
...
func entityResolverNameForFlight(ctx context.Context, rep EntityRepresentation) (string, error) {
	...
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"number\" for Flight", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		m = rep
		val, ok = m["departureDate"]
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"departureDate\" for Flight", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
