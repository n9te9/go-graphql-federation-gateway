...
func (ec *executionContext) resolveEntity(
	...
	switch typeName {
	case "Hotel":
		resolverName, err := entityResolverNameForHotel(ctx, rep)
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "Hotel": %w`, err)
		}
		switch resolverName {
		...
		case "findHotelByID":
			...
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findHotelByID(): %w`, err)
			}
			entity, err := ec.resolvers.Entity().FindHotelByID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "Hotel": %w`, err)
			}
...
func entityResolverNameForHotel(ctx context.Context, rep EntityRepresentation) (string, error) {
	...
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for Hotel", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for Hotel", ErrTypeNotFound))
			break
		}
		return "findHotelByID", nil
	}
	return "", fmt.Errorf("%w for Hotel due to %v", ErrTypeNotFound,
